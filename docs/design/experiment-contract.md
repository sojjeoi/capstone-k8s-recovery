...
**`proposed`의 예측 모델(Isolation Forest) 주장 범위(2026-09-21, §79 - anomaly-detection/v3 모델 도메인 재정의에 따른 명시적 제약, 상세는 `phase8-blue-green-preflight-incident.md` §78~§79):**
- **허용되는 서술**: "Isolation Forest는 CPU·메모리·KV-cache 추세를 이용해 fault-induced multivariate telemetry anomaly를 탐지한다." 예측 경로의 역할은 이 범위로 한정한다 - 반응형 Alertmanager fallback 경로(위 두 arm 공통)가 별도로 latency SLO 위반에 대응한다.
- **금지되는 서술**: "모든 SLO 위반을 사전에 탐지한다", "모든 latency spike의 원인을 탐지한다", "root cause를 판별한다" - §78 오프라인 재감사에서 무장애·정상 인프라(idle/low_load)에서도 latency-only SLO 위반이 자연 발생함을 확인했고(세션 단위 자연 발생률 low_load 12.5%, 95% CI [0.3%, 52.7%]), 이 현상은 CPU/CFS-throttle 등 현재 8-feature로 통계적으로 구분되지 않았다(§78 LOSO). 이런 latency-only 변동을 Isolation Forest가 반드시 탐지해야 하는 positive label로 주장하지 않는다.
- 이 제약은 `proposed` arm의 논문·보고서 서술에 적용되며, 60회 본 실험의 arm 정의·성공조건·스키마(§1~§5)는 바꾸지 않는다.
...
- 2026-09-21: `anomaly-detection/v3` Isolation Forest 모델 라인(memory_pressure calibration과 무관한 별도 트랙, `TrialResult`·60회 본 실험 스키마 변경 없음)에서 §60~§78에 걸친 오프라인 qualification/collection/재감사 끝에 v3.1 모델이 sealed holdout에서 실패(§71~75, false-signal episode 5건·FPR 40%, 미채택·`v31 rejected model`로 영구 보존)했고, 이어진 v3.2 시도의 calibration 수집도 진짜 sustained SLO 위반으로 중단됐다(§76~77, `calib2-low-02`, 미완료 상태로 보존, 재개하지 않음). §78 오프라인 재감사(SLO 재판정 12/12 일치, session 단위 정확 이항신뢰구간, 교정된 LOSO)로 이 반복되는 "무장애 세션에서의 latency-only SLO 위반"이 harness 결함도 아니고 현재 8-feature로 뚜렷이 구분되지도 않는 "B: latency-only 미설명 위반"임을 확정하고, "SLO 위반 세션은 정상 데이터에서 뺀다"는 기존 관행이 survivorship bias였을 위험을 지적받았다. §79에서 `proposed` 예측 모델의 선언된 역할을 **fault-induced multivariate telemetry anomaly 탐지**로 명시 축소하고(latency/TTFT는 추가하지 않고 Phase 8 이후 ablation 후보로만 기록), 정상 데이터 정의를 **infrastructure-normal**(chaos/restart/OOM/Node/Endpoint/promotion/성공률/메트릭 완전성 - 학습 포함 기준)과 **SLO label**(`t_slo` 유무 - 별도 결과 필드, 포함 기준 아님)로 분리한 v3.2b를 새로 시작했다(코드: `anomaly-detection/v3/model_v32b/domain.py`·`train_v32b.py`·`calibrate_v32b.py`·`evaluate_holdout_v32b.py`, 테스트 14개 추가로 전체 스위트 792 passed). Training 9세션(idle 4+low_load 5, `calib2-low-02`처럼 `t_slo`가 있어도 infrastructure-normal이면 포함, 337 rows), 신규 Calibration 6세션(`calib3-*`)·Prospective Holdout 6세션(`holdout3-*`, freeze commit push 후에만 수집) 순서를 사전 등록했다. calibration false-signal 판정은 `t_slo`와 무관하게 순수 score 상태기계 기준으로 유지(§79.6, `replay.calibrate_threshold()` 코드 변경 없음), holdout stop-loss 규칙은 v3.2의 `decide_holdout_outcome()`을 그대로 재사용(1건이라도 false-signal episode면 무조건 미채택, 재시도·재조정 없음). 위 "허용/금지 서술" claims-scope 제약을 이 문서 §1(`proposed` 표 아래)에 명시했다. 이번 커밋은 정의·코드·테스트·문서 등록까지만이다 - `calib3-*` 실측 수집·v3.2b 모델 학습·freeze·holdout 평가는 아직 하지 않았다. 상세는 `phase8-blue-green-preflight-incident.md` §78~§79 참고.
...
- 2026-09-20: `memory_pressure` 최종 후보(500/1000/1500MB, 각 120초, native, worker 1) 3회 재현성 검증 사전 등록(`phase8-blue-green-preflight-incident.md` §54) - 안전 상한(5GiB)·1600MB·1650MB는 이번에도 손대지 않는다. 신규 `experiments/verify_memory_pressure_candidate.py`: 세 stage를 어댑터의 기존 다단계 `stages` 리스트 기능으로 **하나의 progressive 시퀀스**로 실행(`run_memory_pressure_trial.py`의 1GB 이상 차단은 불변, 완전 별도 경로), stage별 실제 시작/종료 시각(`Injector.get_stage_windows`, 신규 선택 훅 1개)으로 probe raw CSV·안전 tick을 나눠 분석해 이전 stage 잔여효과나 명목 경계로 오분류되지 않게 했다. stage별 AllInjected 확인을 위해 `memory_pressure_adapter.py`의 기존 5초 안전 tick 루프에 `is_stage_injected()` 확인을 얹었고(추가 대기 없음, 기존 안전/중단 로직 불변), 안전 로그 콜백 경로에도 실제 시각을 통일해서 찍도록 `_log()`를 고쳤다(파일 기록 경로는 이미 있었음 - 비대칭 해소). `explore_memory_pressure_intensity.analyze_slo()`에 `upper_bound_iso` 선택 인자를 추가해 stage 경계 안에서만 SLO를 판정한다(기존 호출부는 인자 안 넘기므로 동작 불변). 사전 등록 기준: 안전 9개(모든 stage AllInjected·completion 성공률 100%·restart 불변·OOM 없음·Node 정상·MemAvailable≥4GiB·working set<5GiB·CR·observer 정리·최종 recovery)를 3회 모두 충족 + stage별 SLO(500MB·1000MB는 3회 모두 미위반, 1500MB는 최소 2/3회 sustained 위반 - 순간 P95 초과만으로는 위반 처리 안 함, `slo_judge`의 30초 연속/20표본 기준 그대로 재사용) + 즉시중단 7개(위반 시 해당 반복에서 끝내고 이후 반복 실행 안 함). 기준을 모두 충족하면 `scenario-progressive-memory-pressure.yaml` 최종 구성을 500/1000/1500MB×120초로 동결하고 "실제 OOM 유도" 표현을 제거하기로, 충족 못 하면 강도·시간을 즉석 조정하지 않고 결과만 보고하기로 사전 확정했다. **`TrialResult` 스키마·기존 단일 라운드 도구(§50/§52) 동작 불변**. 오프라인 테스트 24개 추가(`test_verify_memory_pressure_candidate.py` 19개 + 어댑터·analyze_slo 확장 검증 5개), 전체 스위트 640 passed(직전 616에서 +24). memory_pressure 3-arm 파일럿·`run_all_scenarios.py`·본 실험·1600MB 이상 탐색·재현성 반복 자체 전부 아직 미실행.
- 2026-09-20: `memory_pressure` 최종 후보(500→1000→1500MB, 각 120초 progressive) 3회 재현성 검증 실행 결과(`phase8-blue-green-preflight-incident.md` §55) - **안전 기준 9개는 3회 모두 PASS**(모든 stage AllInjected, completion 성공률 100%, restart/OOM 없음, Node 정상, MemAvailable 6.4GiB 이상, working set 5.08GiB대(상한까지 ~290MiB 여유), 최종 recovery baseline 복귀 확인), 그러나 **§54.5의 1500MB stage SLO 재현성 기준(최소 2/3회 sustained 위반)은 미충족(0/3회)** - 500MB·1000MB는 사전 등록대로 3회 모두 미위반. 실행 중 `analyze_slo()`의 stage별 `p95_peak`/`availability_min`/표본수 스코핑 버그를 발견·즉시 수정했다(같은 라운드의 세 stage에 raw CSV 전체 기준 값이 똑같이 찍히던 문제 - `t_slo`/`t_recovery` 판정 자체는 이미 올바르게 stage 하한이 걸려 있어 최종 판정에는 영향 없음, `upper_bound_iso`를 안 넘기는 §50~§53 기존 호출부는 계산 범위 불변이라 그 문서 수치도 안 바뀜, 회귀 테스트 1개 추가, 전체 641 passed). 보존된 raw CSV로 재계산한 stage별 `p95_peak`는 3회 전부 3 stage 모두 0.34~0.36초(SLO 임계치 0.648초의 약 55%)로, §53의 단독 1500MB×120초 라운드(`p95_peak=1.124초`, sustained 위반 확정)와 정면으로 배치된다 - 유일한 구조적 차이는 1500MB 도달 경로(§53은 baseline에서 곧장 점프, 이번은 500→1000MB 선행 압박 240초 후 도달)이며, 두 가설(선행 압박에 의한 적응 효과 / §53 자체가 30초 조건에 겨우 걸친 경계선 사례) 중 어느 쪽인지는 판단하지 않고 결과만 보고했다. **§54.8 지시대로 기준 미충족 시 조정 없이 보고만 함**: `scenario-progressive-memory-pressure.yaml` 최종 구성 동결 안 함, "실제 OOM 유도" 표현 제거·1500MB의 "안전한 high-stage" 확정 안 함, 강도·지속시간 즉석 조정 없음. **안전 상한·`TrialResult` 스키마 변경 없음**. memory_pressure 3-arm 파일럿·`run_all_scenarios.py`·본 실험·추가 반복 전부 미실행.
- 2026-09-20: Isolation Forest 모델 감사 결과 공식 결정 + v3 정상 데이터 파이프라인 설계(`phase8-blue-green-preflight-incident.md` §58) - **현재 모델(19×8 matrix, 독립 holdout·FPR 없음, 4코어 학습 후 3코어 전환 이후 한 번도 재학습 안 됨)은 본 실험용으로 채택 불가**로 공식 결정. 기존 `proposed` 파일럿 3건은 배선·탐지→promotion 경로 검증용 제외 pilot로만 유지, 성능·우위 근거로 재사용 금지. 본 실험 전 3코어 기준 모델 재학습·검증·동결 필요, memory_pressure negative-control 3-arm과 본 실험은 동결 이후에만 진행하기로 결정. 신규 `anomaly-detection/v3/`(v1인 `data/regimes.jsonl`/`artifacts/*.pkl`은 이력 보존, 손대지 않음): `features.py`의 PromQL·계산식을 그대로 재사용해 score_server.py와 동일한 고정 60초 트레일링 윈도우·15초 스텝으로 정상 데이터를 만들고, 지표 하나라도 결측이면 0으로 채우지 않고 창 전체를 invalid 처리하며, (regime, topology) 조합별로 세션 단위 층화 split(seed 고정)을 한다. `arm_controller.py`/`run_once.py` 코드 근거로 topology를 조사한 결과 **`proposed`/`fixed_threshold`의 detector는 실제로 `active_plus_preview`(active+preview 동시 Ready) 상태에서 동작하는데, v1의 19개 표본 중 그 topology는 1개뿐**이었다는 구조적 편향을 발견했다. Prometheus read-only 조회(retention 30일 확인, 워크로드·Chaos 없음)로 3코어 전환 이후 기존 파일럿·탐색 baseline에서 12개 독립 세션(active_plus_preview 6, active_only 6)을 찾아 58개 유효 feature row를 산출했다(재학습·threshold 결정 없음, inventory만) - `memory_mean`이 3.57e9~7.50e9로 topology에 따라 약 2배 차이 나는 것을 실측 확인했고, `queue`뿐 아니라 `cache`까지 이번 12세션 전부에서 완전히 0(상수)임을 발견했다(v1에서는 cache에 작은 변화가 있었음 - 더 무거운 부하가 이번 후보에 없었기 때문일 가능성, 미확정). 목표 데이터 기준·부족 regime/topology별 live 수집 시간 추정·향후 train/calibration/holdout 검증 계획(threshold는 calibration에서만 정하고 holdout으로 재조정 안 함)을 제안만 하고 실행하지 않았다. 오프라인 테스트 18개 추가, 전체 스위트 681 passed(직전 663에서 +18). **재학습·threshold 변경·`model.pkl`/`scaler.pkl` 교체·`score_server.py` 런타임 변경·workload/Chaos 실행·preview 생성·promotion 전부 없음.**
- 2026-09-20: `active_plus_preview` topology 정상 데이터 live 수집 사전 등록(`phase8-blue-green-preflight-incident.md` §59, `anomaly-detection/v3/collection_manifest.json`) - §58.6의 최대 공백(그 topology의 low_load/sustained_load/burst 세션 0개)을 메우기 위해 qualification(1 regime당 1회, 총 3회) → 통과 시에만 official(1 regime당 3회, 총 9회) 순서로 진행하기로 확정. 강도는 새로 고르지 않고 load_ramp calibration이 이미 SLO 준수로 확인한 RPS(0.10/0.50)만 재사용 - burst는 강도가 아니라 지속시간(90초 vs sustained_load 300초)으로만 구분하고, v1 `regime_configs/burst.yaml`의 2 RPS(이번 calibration 기준 미검증)는 재사용하지 않는다. SLO 정의·임계치 변경 없음. 신규 `anomaly-detection/v3/collect_session.py`: `experiments/blue_green_prep.py`의 `prepare_preview_with_rollback()`/`cleanup_unpromoted_preview()`(모든 non-native 파일럿이 이미 쓰는, promotion 없이 Ready까지만 준비하고 abort+단일 revision 복원을 실측 재확인하는 기존 경로)와 `explore_ramp_intensity.py`의 `run_candidate()`/`check_node_and_pods()`(ramp+probe 동시 실행, 기존 SLO violates 판정)를 그대로 재사용 - 새 클러스터 조작 코드 없음. 세션 하나 = regime 하나(연속 측정으로 독립 세션 수를 부풀리지 않음), train/calibration/holdout은 `(regime, topology)` 층화 세션 단위 split(§58.2와 동일 seed=20260920), sustained SLO 위반·restart·OOM·Node 이상·promotion·target replacement·cleanup 실패 중 하나라도 있으면 제외, qualification과 official 자료는 `is_pilot`/`included_in_training` 플래그로 구조적으로 분리. 오프라인 테스트 10개 추가, 전체 스위트 691 passed(직전 681에서 +10). **실제 live 측정은 이 사전 등록 커밋 이후 별도 진행 - 아직 preview 생성·workload 실행 없음.**
- 2026-09-20: `low_load` qualification 결과 - 예상 밖 SLO 위반으로 §59.2 규칙대로 정지(`phase8-blue-green-preflight-incident.md` §60). 측정 자체를 완주시키기까지 인프라 버그 2건을 발견·수정했다: (1) `blue_green_prep.py`의 `created_pod_hash` 캡처 시점 race condition(모든 non-native 파일럿이 이미 쓰던 함수, bump 직후 곧바로 읽으면 직전 시도의 잔여값을 돌려받을 수 있었음 - Ready/timeout 시점에 재확인하도록 수정), (2) `collect_session.py`의 pod 이름에 밑줄 포함(`"low_load"[:5]="low_l"`이 label에 그대로 들어가 K8s 이름 규칙 위반으로 6회 연속 100% 재현 - 진짜 원인, 하이픈으로 치환해 수정). 수정 후 완주된 결과: preview+active 동시 서빙 중 0.10 RPS(load_ramp calibration 확인 안전값) stage에서 baseline(P95 0.311초)·drain(P95 0.318초)은 정상인데 stage 구간만 **P95 12.654초**(요청 성공률 100%, 그러나 극심한 지연)로 위반 - CPU는 0.81~1.86코어(합산 한도 6코어·Node 총 8코어에 한참 못 미침), queue/cache는 여전히 거의 0이라 단순 CPU 포화·큐잉으로는 설명 안 됨. 원인은 확정하지 않고 관측만 기록했다. 지시대로 `sustained_load`·`burst` qualification과 official 수집을 진행하지 않고 정지, 강도·설정 조정 없음. 사후 kubectl 독립 확인으로 클러스터 정상 복원 확인.
- 2026-09-20: `analyze_slo()` stage 스코핑 버그 수정에 회귀 테스트 4개 추가(합성 데이터로 "stage 경계 안 표본만 사용"·"서로 다른 stage가 서로 다른 p95_peak 반환"·"t_slo/t_recovery 판정 로직 불변" 확인 + §54 3회 재현성 검증에서 실제로 수집된 raw CSV·요약 JSON을 `docs/design/evidence/memory-pressure-candidate-verify/`에 fixture로 보존해 §55.2 문서 수치와 정확히 일치하는지 고정). §55의 최종 판정을 공식화(§55.6): progressive 후보 재현성 FAIL, 안전성 PASS, 1500MB stage sustained SLO 위반 0/3, 최종 시나리오로 동결 안 함, §53과의 차이 원인은 인과 결론 없이 `path-dependent behavior observed`로만 기록. 전체 오프라인 스위트 645 passed(직전 641에서 +4).
- 2026-09-20: memory_pressure "direct"(단일 점프) 후보 3회 재현성 검증 사전 등록(`phase8-blue-green-preflight-incident.md` §56) - progressive 없이 정상 baseline에서 곧장 1500MB×120초로 주입(§53과 동일 조건)하는 후보의 재현성을 확인한다. 신규 `experiments/verify_memory_pressure_direct_candidate.py`: 새 어댑터·주입 경로 없이 `explore_memory_pressure_intensity.run_round(1500.0, stage_duration_sec=120.0)`를 그대로 재사용(§53이 이미 이 호출이었음), `verify_memory_pressure_candidate.py`의 quiescence 헬퍼 재사용, 3회 반복+cooldown. `target UID 불변` 명시 확인을 위해 `run_round()`에 `target_replacement`(선택 훅 결과) 필드를 추가했다 - 기존 §50~§53 호출부는 이 키를 안 읽으므로 동작 불변. 안전 기준 10개(AllInjected·completion 성공률 100%·working set<5GiB·MemAvailable≥4GiB·restart 불변·OOM 없음·Node 정상·target UID 불변·cleanup 후 baseline 복귀·CR·observer·context 정리)를 3회 모두 충족해야 하고 한 번이라도 위반하면 즉시 중단. SLO 재현성은 3회 중 최소 2회 sustained 위반(`t_slo not None` AND `t_slo_within_window` - §55.2와 같은 원칙으로 stage 경계 밖 오분류 방지), 20표본·30초 연속·availability 불필요 기준은 `slo_judge` 그대로 재사용. 기준 충족 시 신규 `sudden_memory_pressure` 단일 step 시나리오 동결(progressive YAML은 이력 보존), 미달 시 강도·상한·SLO 변경 없이 보고만 하기로 사전 확정했다. **`TrialResult` 스키마·안전 상수·기존 §50~§54 도구 동작 전부 불변**. 오프라인 테스트 18개 추가, 전체 스위트 663 passed(직전 645에서 +18). 실클러스터 작업은 이 사전 등록 커밋 이후 별도 진행.
- 2026-09-20: memory_pressure "direct"(단일 점프) 후보 3회 재현성 검증 실행 결과(`phase8-blue-green-preflight-incident.md` §57) - **안전 기준 10개는 3회 모두 PASS**(AllInjected, completion 성공률 100%, restart/OOM 없음, target UID 불변, Node 정상, MemAvailable 6.4GiB 이상, working set 5.076~5.078GiB(§53과 사실상 동일), cleanup 후 baseline 복귀), 그러나 **§56.4 SLO 재현성 기준(최소 2/3회)은 미충족(0/3회)** - §53의 유일한 sustained 위반 관측이 동일 조건 3회 반복에서 재현되지 않았다. 흥미로운 발견: 전체 라운드 기준 `p95_peak`(0.95~1.30초, §50~§53과 같은 계산)와 stage 경계로 좁힌 `p95_peak`(0.33~0.36초)의 차이가 컸다 - 순간적 고지연이 주입 구간이 아니라 baseline/recovery/drain에서 발생했다는 뜻이며, `t_slo`는 둘 다 null이라 이번 판정 자체엔 영향 없음. 보존된 raw CSV로 독립 재계산해 3회 전부 부동소수점까지 정확히 일치함을 확인했다. §55(progressive)에 이어 §56(direct)도 재현성 기준 미충족 - **§56.5 지시대로 강도 상향·1650/2000MB·안전 상한·SLO 정의 변경 전부 하지 않고**, "안전 범위 안에서 memory pressure가 현재 SLO를 안정적으로 재현하지 못한다"고 판정했다. sub-critical 유지 여부·본 실험 제외 여부는 판단하지 않고 사용자 결정으로 남겼다. `scenario-progressive-memory-pressure.yaml`도 신규 `sudden_memory_pressure`도 동결하지 않았다. **안전 상한·`TrialResult` 스키마·SLO 정의 변경 없음**. memory_pressure 3-arm 파일럿·`run_all_scenarios.py`·본 실험 전부 미실행.
- 2026-09-20: §60 `low_load` qualification 지연 급증(P95 12.654초) 원인·재현성 분리 - 오프라인 포렌식 + A-B-A 진단 사전등록(`phase8-blue-green-preflight-incident.md` §61, `anomaly-detection/v3/diagnostics/aba_manifest.json`). 포렌식(요청 payload/RPS/generator 개수/active-preview Service 격리/settle 시간 대조, Prometheus 이력 조회 - workload/Chaos 미실행)에서 하니스 스펙 오류는 못 찾았고, 대신 stage 구간에서 active pod 자신의 CFS throttle 비율이 idle 3%→50~60%대로 급상승(CPU idle 0.1~0.6코어→2.0~2.25코어와 시간 일치, Node 8코어 전체 사용률은 33~43%로 포화 아님)함과, 원래 "0.10 RPS 안전" calibration(2026-09-16)이 CPU 4→3코어 cutover(2026-09-18)보다 먼저 끝나 어떤 topology에서도 3코어 한도 하에 재검증된 적이 없다는 교란 변수 후보를 발견했다 - 측정 오류가 아니므로 코드를 고치지 않고 A1(active-only)→B(active_plus_preview, 60초 이상 settle)→A2(abort 후 active-only) 진단을 실측 전 사전등록했다. 신규 `anomaly-detection/v3/diagnostics/`(새 클러스터 조작 코드 없음 - `blue_green_prep`/`explore_ramp_intensity`/`collect_session.run_candidate_with_retry` 재사용): `aba_diagnostic.py`(`--leg {a1,b,a2}`), `aba-low-load.yaml`(payload는 `low-load.yaml`과 동일, duration만 90초 - 강도 변경 아님). 전량 diagnostic pilot, 학습·threshold 결정 제외. 판정 규칙(A1·A2 정상+B만 재현 / 셋 다 느림 / B 정상+기존만 이상 / 즉시중단 조건)을 측정 전에 확정.
- 2026-09-20: A-B-A 진단 실측 결과(`phase8-blue-green-preflight-incident.md` §62) - A1→B→A2 순서·강도(0.10 RPS) 변경 없이 전량 실행, 세 구간 모두 `target_replaced=False`·stop 조건 없음·B의 abort 후 단일 revision 복원 확인(`abort_and_rollback_ok=True`). **§60의 P95=12.654초급 지연 급증은 A1·B·A2 어디에서도 재현되지 않았다** - 세 구간 stage P95가 0.628~0.683초로 SLO v3 threshold(0.648초, `L_BASELINE=0.324`의 2배) 근처에 몰려 근소하게(±0.02~0.035초) 오르내렸을 뿐이다(stage당 9표본이라 표본 변동 범위로 봄). Prometheus 이력 조회 결과 **active pod 자신의 CPU 사용량(avg 1.46~1.67코어)과 CFS throttle 비율(avg 34~37%, max 51~52%)이 A1·B·A2 세 구간에서 사실상 동일** - preview가 존재하는 B에서도 preview 자신의 CPU는 무시할 수준(0.01~0.02코어)이고 throttle은 0%였다. 즉 이 근소한 CPU 배분 현상은 preview 존재 여부와 무관하게 active-only(A1)에서도 동일하게 나타나 **active_plus_preview topology가 원인이라는 증거는 나오지 않았다.** 메모리는 topology 차이만큼만 다름(active pod 세 구간 모두 3.47GiB 동일, B에서만 preview가 3.51GiB 추가 - 원인으로 해석 안 함). 사전등록한 4갈래 판정 중 두 개("B 정상+기존만 이상 → non-reproduced diagnostic anomaly로 §60 원본 유지" / "셋 다 근소하게 느림 → topology 아닌 환경·워크로드 변동, 특히 3-core cutover 이후 미검증 상태")를 확대 해석 없이 병기해 기록했다. §60 원본 기록은 삭제하지 않았고, 공식 정상 데이터 수집·모델 재학습은 계속 중지한다. **모델 재학습·threshold 변경·artifact 교체·`sustained_load`/`burst`/공식 9세션 수집·`memory_pressure` 3-arm·`run_all_scenarios.py`·본 실험 전부 없음.**
- 2026-09-20: §61/§62 공식 결정 확정 + 3-core 정상 부하 profile qualification 사전등록(`phase8-blue-green-preflight-incident.md` §63, `anomaly-detection/v3/profile_qualification_manifest.json`) - §60은 `non-reproduced diagnostic anomaly`로 보존하고 `active_plus_preview` topology 자체의 성능 간섭 원인 결론은 내리지 않음(A-B-A에서 A1/B/A2가 preview 유무와 무관하게 CPU/CFS throttle/latency가 사실상 동일했으므로 "확인되지 않음"으로 기록). 기존 `low_load=0.10 RPS`는 4-core calibration(2026-09-16) 값이라 v3 학습 profile에서 제외. 새 후보 `low_load=0.025`/`sustained_load=0.05`/`burst=0.025 base+0.10 pulse×4(pulse 20초, base 60초×5)`를 `active_plus_preview`에서 qualification(공식 수집 아님)하기로 확정 - pulse 길이 20초는 `slo_judge.LATENCY_PERSIST_SEC`(30초) 미만으로 고정해 pulse 하나가 30초-sustained 판정을 구조적으로 인위 충족하지 않게 설계. 신규 `anomaly-detection/v3/qualify_normal_profile.py`: 새 클러스터 조작 코드 없이 `blue_green_prep`/`explore_ramp_intensity.run_candidate()`/`build_dataset.build_rows_for_session()` 재사용, `run_candidate()`의 stage별 순간 P95 판정이 아니라 `slo_judge.evaluate()`/`find_t_slo()`로 probe raw 전체에 걸친 진짜 30초 sustained 위반만 PASS/FAIL 기준(`t_slo`)으로 삼음, 신규 `check_endpoint_isolation()`(Service selector가 아니라 실제 Endpoints 객체로 active/preview 격리 확인), `prometheus_session_summary()`(CPU/CFS throttle/Node 지표 - 보고용, PASS/FAIL 기준 아님), §60 재발을 30초-sustained 기준과 별개로 잡는 `extreme_latency_detected`(P95>threshold×5 또는 max>threshold×10). 오프라인 테스트 16개 추가, 전체 스위트 719 passed(직전 703에서 +16).
- 2026-09-20: 3-core 정상 부하 profile qualification 실측 결과(`phase8-blue-green-preflight-incident.md` §64) - `low_load→sustained_load→burst` 순서로 각 1회, 세션 사이 60초 cooldown, **세 profile 전부 PASS**(`t_slo=null`, `extreme_latency_detected=False`, endpoint 격리 유지, cleanup 성공, 무효 window 0개). 실행 중 오프라인에서 즉시 발견해 클러스터 손대기 전/직후에 수정한 코드 버그 2건(`EXPERIMENTS_DIR` 경로 계산 오류로 `slo_judge` import 실패, Prometheus 요약 구간 계산에서 이미 datetime인 값을 `fromisoformat()`에 다시 넣어 TypeError - `low_load` 1차 시도가 측정·cleanup은 정상 완료한 채 결과 저장 직전에 죽어 원본 CSV만 보존한 채 처음부터 재실행함) - 각각 수정·오프라인 재검증(719 passed) 후 커밋, 이후 세 profile은 한 번에 통과. stage P95: `low_load` 0.424초, `sustained_load` 0.592초, `burst` base 0.58~0.62초/pulse 0.625~0.698초(pulse 4개 중 3개가 `run_candidate()`의 순간 P95 판정으로는 threshold를 근소하게 넘었으나 - 인위적으로 막은 게 아니라 실제로 발생 - `slo_judge.find_t_slo()`의 30초 연속 판정은 세 profile 전부 위반 없음으로 확인, pulse 20초 설계가 의도대로 동작함을 실측 검증). Prometheus 이력 조회 결과(보고용): active pod의 CFS throttle 비율이 세 profile 모두 27~49%대로 §61/§62와 동일하게 관측됐지만(RPS를 낮춰도 이 배경 현상 자체는 사라지지 않음) latency는 threshold 안쪽 또는 momentary 근소 초과에만 머물렀다. `queue_mean`은 세 profile 전부 상수 0을 유지했지만, **`cache_mean`이 `sustained_load`·`burst`에서 처음으로 상수 0이 아니었다**(이번 investigation 전체를 통틀어 최초 관측, 원인 미판단). 결론: 세 profile이 공식 9세션 수집으로 넘어갈 준비는 됐다고 판단하나, 이번 범위(§63.7)에는 공식 수집을 포함하지 않아 아직 시작하지 않았다 - 참고사항으로 이번 qualification은 profile당 1회뿐이라 §59와 같은 최소 2/3회 재현성 기준은 아직 미적용임을 명시. **모델 재학습·threshold 변경·artifact 교체·공식 9세션 수집·`memory_pressure` 3-arm·`run_all_scenarios.py`·본 실험·`TrialResult` 스키마 변경·Chaos 주입/promotion 전부 없음.**
- 2026-09-20: 공식 v3 정상 데이터 9세션 수집 사전등록(`phase8-blue-green-preflight-incident.md` §65, `anomaly-detection/v3/official_collection_manifest.json`) - §63/§64 세 profile을 3-core qualification 통과로 확정, Latin square 순서(Block1=low_load→sustained_load→burst=train, Block2=burst→low_load→sustained_load=calibration, Block3=sustained_load→burst→low_load=holdout)의 공식 9세션(session_id·역할 측정 전 고정, 결과 보고 후 교환 안 함)을 계획. `qualify_normal_profile.py`를 `--official --split-role`로 확장(§63과 완전히 동일한 세션 로직 재사용) - `git_commit_sha`/config SHA-256 기록, `window_boundary_ok` PASS 조건 추가. §58의 `active_plus_preview` 6세션을 strict completeness 재검증(32/32 valid) 후 알파벳순+라운드로빈으로 idle regime train/calibration/holdout 역할 사전 배정, `active_only` 6세션은 topology 불일치로 제외(out-of-domain 참고 자료로만 보존). 오프라인 테스트 2개 추가, 전체 721 passed.
- 2026-09-20: 공식 v3 수집 세션 1(`official-train-low_load-20260920`) PASS, 세션 2(`official-train-sustained_load-20260920`)에서 진짜 30초 sustained SLO 위반(`t_slo` not null) 발생으로 §65.4 규칙("sustained SLO 위반이면 이후 세션 중단, 강도 조정·대체 세션 금지")에 따라 즉시 정지(`phase8-blue-green-preflight-incident.md` §66) - 세션 3~9 미실행. 세션 2는 stage 전체(300초) 평균 P95=0.571초로 threshold(0.648초) 아래였지만(`run_candidate()`의 순간/평균 판정으로는 놓쳤을 사례), `slo_judge`의 진짜 30초 rolling-window 판정이 stage 구간 안의 국소적 위반(10:54:20 부근)을 정확히 잡아냈다 - §63.4에서 사전등록한 방법론이 의도대로 작동함을 재확인. `extreme_latency_detected=False`(§60 수준의 파국적 규모 아님). 어제(§64) 같은 강도의 qualification은 PASS했으나 오늘 독립 2차 측정은 FAIL해 sustained_load profile의 재현 안정성이 아직 확인되지 않았음을 뜻한다(원인 미판단). 강도 조정·대체 세션 없이 원본 보존, cleanup 정상 완료·클러스터 정상 복원 확인. 목표 9세션 중 1개만 확보돼(각 regime 3개 원칙 미충족) 데이터 감사(§65.6)는 수행하지 않음. **모델 재학습·threshold 결정·강도 조정 전부 없음.**
- 2026-09-20: 공식 데이터 계획 개정(`phase8-blue-green-preflight-incident.md` §67, `anomaly-detection/v3/official_collection_manifest_v2.json`) - `sustained_load=0.05 RPS`를 v3 정상 데이터 최종 후보에서 영구 제외(§66의 진짜 sustained SLO 위반 - 강도를 낮춘 대체 탐색도 범위 밖), 최종 후보를 `idle`(6)/`low_load`(3)/`burst`(3)=12개 독립 `active_plus_preview` 세션으로 확정. §65 원본 9세션 계획은 종료된 사전등록 이력으로 그대로 보존. 신규 `classify_official_session()` 순수 함수(오프라인 테스트 5개)로 기존 세션 2건에 `included_in_{training,calibration,holdout}`/`classification` 필드를 소급 반영(재측정 없음) - `official-train-sustained_load-20260920`은 `unexpected_slo_violation`으로 영구 분류(대체 데이터로 계산 안 함, 모델·threshold 동결 후에만 `external_anomaly_validation` 후보로 1회 검토 가능), `official-train-low_load-20260920`은 `normal_valid` 유지. 남은 5세션(`burst`-train→`low_load`-calib→`burst`-calib→`low_load`-holdout→`burst`-holdout) 순서·역할을 측정 전 고정. 오프라인 테스트 726 passed.
- 2026-09-20: 개정 계획 실행 결과(`phase8-blue-green-preflight-incident.md` §68) - 세션 1(`official-train-burst-20260920`) PASS, 세션 2(`official-calib-low_load-20260920`) PASS, **세션 3(`official-calib-burst-20260920`)에서 `burst`도 진짜 30초 sustained SLO 위반**(`t_slo=2026-09-20T12:07:00`) 발생으로 §67.5 규칙대로 즉시 정지 - 세션 4~5(holdout 2개) 미실행. `pulse-4`(순간 위반)→`base-5`(순간 위반) 구간 사이에서 latency가 threshold 위에 30초 이상 머물러 진짜 sustained 위반 조건을 만족시켰다(`extreme_latency_detected=false`, §60 규모 아님). `burst`는 독립 시도 3회 중 2회 PASS(§64 qualification, `official-train-burst`)·1회 FAIL(`official-calib-burst`)로 - `sustained_load`(1/2 FAIL로 영구 제외)보다는 나으나 매번 안정 재현되는 profile은 아직 아님(원인 미판단). 강도·pulse 조정·대체 세션 없이 원본 보존(`classify_official_session()`이 자동으로 `unexpected_slo_violation` 분류), cleanup 정상 완료·클러스터 정상 복원 확인. 목표 12세션 중 이번 라운드에서 `low_load` train/calibration 2개, `burst` train 1개만 새로 확보돼 데이터 감사는 수행하지 않음. **모델 재학습·threshold 결정·강도 조정 전부 없음.**
- 2026-09-20: v3.1 최종 정상 데이터 재설계(`phase8-blue-green-preflight-incident.md` §69, `anomaly-detection/v3/official_collection_manifest_v31.json`, `boundary_challenge_manifest.json`) - `official-calib-burst`의 진짜 sustained SLO 위반(§68)으로 `burst`도 `sustained_load`와 동일하게 v3 정상 domain에서 전체 제외(PASS 2개를 골라 쓰지 않음). `sustained_load` 2세션+`burst` 3세션+§60 세션=6개를 `boundary_challenge_set`으로 재분류(원본 raw data·timestamp 불변, 소급 메타데이터 패치만 - `included_in_{training,calibration,holdout}` 전부 false 강제). 최종 정상 domain을 `idle`(ramp 없이 probe만, 600초)/`low_load`(0.025 RPS, 600초) 2-regime으로 재설계 - §58 idle 6세션 배정도 함께 superseded. `qualify_normal_profile.py`를 `--v31`로 확장, 새 `run_idle_session()`(ramp pod 없이 probe만 실행)만 추가하고 나머지 검증 로직은 전부 재사용. 오프라인 테스트 4개 추가, 전체 730 passed.
- 2026-09-20: v3.1 6세션 전부 PASS + train/calibration 데이터 감사(`phase8-blue-green-preflight-incident.md` §70) - `idle`/`low_load` 어느 쪽에서도 sustained SLO 위반 재현 안 됨(`t_slo=null` 6/6). 1차 시도(`v31-train-idle`)가 `run_idle_session()`의 `kubectl cp` Windows 절대경로 오류로 측정 시작 전 실패(클러스터 부작용 없음, `os.path.relpath()`로 수정 후 재시도해 통과). Train+calibration 4세션(독립, 겹침 없음) 총 150행(idle 76+low_load 74) - 예상 148행과 근소한 차이(+2, idle의 20초 여유 때문). `queue_mean`/`queue_slope`는 150행 전부 정확히 0으로 `zero-variance removal candidate`로만 표시(제외는 범위 밖), `cache_mean`/`cache_slope`는 22/150행(14.7%)에서 비영값 관측돼 원자료 보존·자동 제외 안 함. Holdout 2세션은 `sealed_holdout=true`+원본/feature SHA-256만 기록하고 분포·score는 보지 않음. Boundary challenge set(6세션)은 미평가 그대로 유지. **Isolation Forest 학습·threshold 결정·holdout/challenge score 계산 전부 없음.**
- 2026-09-20: 런타임 판정 감사+feature 선택 규칙 사전등록(`phase8-blue-green-preflight-incident.md` §71-72) - `score_server.py`/`policy.py`/`features.py` 전체를 읽고 추정 없이 확정(decision_function, `score<threshold` 엄격한 미만=0.0, 연속 3회 필요, 정상 1회로 리셋, cooldown 60초, 15초 주기, feature/scaler 순서, recovery-policy의 `preview_ready` 게이트는 별도 계층). 신규 `anomaly-detection/v3/model_v31/replay.py`(런타임 상태기계 재생 `replay_detector()` 하나, calibration 탐색 `calibrate_threshold()`)와 `feature_selection.py`(train-only 0분산 제거, NaN/Inf/None fail-closed, near-zero variance는 보고만, 상관관계 제거 로직 자체 없음) - 아직 실제 데이터 미적용. 오프라인 테스트 21개, 전체 750 passed.
- 2026-09-20: v3.1 학습·calibration 실행(`phase8-blue-green-preflight-incident.md` §73) - Train-only feature 선택 재계산(75행/독립 2세션)으로 `queue_mean`/`queue_slope` 0분산 제거·`cache_mean`/`cache_slope`는 train에서도 실제 분산 확인돼 유지. `StandardScaler`+`IsolationForest(n_estimators=100, contamination="auto", random_state=42)`(v1과 동일, 하이퍼파라미터 탐색 없음)를 Train에만 fit, 3회 독립 재학습으로 `model.pkl`/`scaler.pkl`/`threshold.json` byte-identical 재현성 확인. Calibration 2세션에 `calibrate_threshold()` 실행 - 두 session 모두 false signal episode 0인 가장 민감한 threshold `0.013299` 확정(degenerate 아님, point anomaly 9건 실제 관측). 오프라인 테스트 7개 추가, 전체 757 passed. artifact는 아직 커밋 안 함.
- 2026-09-20: **v3.1 model artifact 동결(`v3_model_freeze_commit` = `9caac66`)**(`phase8-blue-green-preflight-incident.md` §74) - `model.pkl`/`scaler.pkl`/`threshold.json`/`feature-schema.json`/`dataset-manifest.json`/`split-manifest.json`/`training-metadata.json`/`requirements-lock.txt`(scikit-learn 1.9.0/numpy 2.5.1/scipy 1.18.1/joblib 1.6.0/threadpoolctl 3.6.0)/`SHA256SUMS.json` 전부 커밋(`.pkl`은 `.gitignore` 예외로 `git add -f`, v1 artifact는 그대로 gitignore 유지). 이 커밋 이후에만 Holdout을 열기로 사전확정.
- 2026-09-20: **Sealed Holdout 평가 - 채택 기준 미충족**(`phase8-blue-green-preflight-incident.md` §75) - freeze commit push 확인(`git rev-list` 결과 "0 0") 후 봉인 해제. `v31-holdout-idle-20260920`은 false signal episode 0(PASS)이었지만 **`v31-holdout-low_load-20260920`에서 최대 연속 6회(3회 임계값 초과)로 false signal episode 5건 발생** - 전체 point FPR 40.00%. 사후 진단(원인 단정 안 함): holdout의 `cpu_mean` 최솟값이 train/calibration 세션보다 뚜렷이 낮아, 독립 session이 2개뿐인 train/calibration으로는 모델이 학습하지 못한 정상 변동 구간일 가능성 - 파이프라인 버그 흔적은 없음(session 중복 없음, 재확인 완료). 지시대로 **모델을 채택하지 않고, threshold 재조정·holdout→calibration 전환·재학습 전부 하지 않음**, 실패 결과(`holdout-evaluation.json`) 그대로 보존. Holdout 실패로 **boundary challenge 평가(§76)는 실행하지 않음** - 새 모델을 채택하려면 새로운 calibration/holdout 데이터(더 많은 독립 session)가 필요하다고만 보고.
- 2026-09-20: Isolation Forest v3.2 최종 데이터 확충 재설계 사전등록(`phase8-blue-green-preflight-incident.md` §76, `anomaly-detection/v3/model_v32/v32_manifest.json`) - v3.1 model(freeze commit `9caac66`)을 rejected model로 영구 보존, threshold 0.013299 재조정 안 함, 기존 holdout 재평가 안 함. Training=v3.1의 idle 3+low_load 3=6세션 전부 재사용(원래 role 무관, 전부 정상 데이터), Calibration/Prospective Holdout 각각 새 6세션(idle 3+low_load 3) 신규 수집으로 재설계. 프로토콜 완전 동일(600초/60초 window/15초 step/0.025 RPS), 수집 도구도 `qualify_normal_profile.py --v31` 그대로 재사용. 신규 `model_v32/`(`train_v32.py`/`calibrate_v32.py`/`evaluate_holdout_v32.py`) - model_v31의 `replay.py`/`feature_selection.py`/`evaluate.py`/`integrity.py`를 프로토콜 공용 유틸로 그대로 import. 신규 `stop_loss.py` - prospective holdout에서 false signal episode 1건이라도 나오면 무조건 거부하고 반복 재시도하지 않는 규칙을 순수 함수로 고정. 과정에서 `train.py`/`calibrate.py`/`evaluate_holdout.py` 이름이 model_v31과 겹쳐 전체 테스트 실행 시 `sys.modules` 캐시 충돌 버그를 발견·수정(`*_v32.py`로 개명). 오프라인 테스트 7개 추가, 전체 765 passed.
- 2026-09-20: 새 Calibration 수집 - 3번째 세션에서 `low_load` 자체의 진짜 SLO 위반 발생, §76.5 규칙대로 즉시 정지(`phase8-blue-green-preflight-incident.md` §77) - `calib2-idle-01`/`calib2-low-01` PASS, `calib2-low-02`에서 stage 전체 평균 P95(0.497s)는 정상이었지만 max=3.135s의 국소적 이상치가 있었고 `slo_judge`의 진짜 30초 sustained 판정이 이를 잡아냈다(§66/§68과 동일 패턴 - 전체 평균은 정상, 국소 구간만 위반). `low_load`는 이번 investigation 전체에서 독립 실행 8회 중 7회 PASS·1회 FAIL을 기록(원인·채택 가능성 결론 없음). 목표 6세션 중 유효 2개만 확보돼 v3.2 학습·calibration·freeze로 진행하지 않음, 강도 조정·대체 세션 없이 원본 보존, 클러스터 정상 복원 확인. **v3.2 학습·calibration·freeze·Prospective Holdout 수집·boundary challenge 평가 전부 없음.**
- 2026-09-20: `idle`/`low_load=0.025 RPS` 무장애 SLO 위반 오프라인 전수 재검증(`phase8-blue-green-preflight-incident.md` §78, 신규 `anomaly-detection/v3/slo_reaudit/`) - 추가 live 측정 없이 보존된 raw CSV·session JSON만 사용. 동결된 `slo_judge`를 12개 세션(idle 4+low_load 8) 전부에 독립 재적용해 원본 기록과 완전 일치 확인(과거 small-sample/baseline gate/timestamp ordering 버그 재발 없음). Session 단위 자연 위반율: idle 0/4(95% CI [0%,60.2%]), low_load 1/8(12.5%, 95% CI [0.3%,52.7%]) - 정확한 Clopper-Pearson 이항 CI. 실패 세션(`calib2-low-02`)의 raw latency는 연속 저하가 아니라 두 번의 짧은 스파이크 뭉치(17:00:12-13, 17:00:28-31)였고 60초 rolling window가 그 잔여 영향을 끌고 가며 66초 스트릭을 만들었음을 확인 - **더 중요하게는 PASS한 7세션 전부 최장 연속 위반 스트릭이 12~22초로 30초 문턱에 근접**해 있어 실패 세션(61초)이 질적으로 다른 별종이 아니라 같은 분포의 꼬리로 보임. Feature 비교: 1차 풀링-quartile 방식은 세션 간 이질성으로 오도됐고(6개 중 4개가 "범위 밖"으로 잘못 나옴), 세션 단위로 교정한 LOSO(`loso.py`)에서는 6개 중 2개는 초과 없음·나머지도 10~40% 수준의 근소한 초과뿐, CPU/CFS throttle 평균도 8세션 전부 사실상 동일. **판정: B(latency-only unexplained violation)** - 현재 feature로 사전 탐지 근거 없음, feature 추가(latency/TTFT)나 주장 축소안을 구현 없이 제안만 함. Survivorship bias 위험을 인정 - 세션 전체 제외 대신 시간 구간 단위 event-aware 설계를 제안만 하고 실제 dataset manifest는 변경하지 않음. 오프라인 테스트 13개 추가, 전체 778 passed. **live 측정·모델 재학습·threshold 변경·artifact 교체·holdout/challenge 평가·기존 raw data 수정 전부 없음.**
