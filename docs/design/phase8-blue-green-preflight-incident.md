...
## §116 - `pod_kill` 공식 재측정 2차 반복 블록(2~3반복, 6건) 실행 - 전부 정상 완료

### 116.1 범위

§115의 1차 반복(3건) 이후, 이번 턴은 사전 등록된 반복 2~3의 6건만
실행: `fixed_threshold-02 → proposed-02 → native-02 → proposed-03 →
native-03 → fixed_threshold-03`(`--from-run-id`/`--to-run-id`로 경계
지정). 반복 4~5, `network_degrade`, memory auxiliary는 시작하지 않음.

### 116.2 사전점검 재확인 (전부 이번 턴에 직접 재확인, 2026-09-24)

- `git`: 시작 시 `HEAD==origin/master==14a5626`, 추적 파일 변경 0건.
- 라이브 클러스터: 두 Node `Ready`+pressure 없음, `Rollout vllm-serving`
  진짜 `phase=Healthy`(activeSelector==previewSelector==
  currentPodHash==stableRS=="84cd9b9fb7", pauseConditions=null,
  §115 3건 실행 후의 pod), 활성 pod 재시작 0회, Chaos CR 잔여 0,
  실험 pod 잔여 0, detector 프로세스 잔여 0.
- 포트포워드 재수립 후 recovery-policy(`{"status":"ok"}`)·
  Prometheus(`Healthy`) 헬스체크 정상, `/admin/quiescent`=true,
  `/admin/experiment-run`=null.
- Prometheus 신선도: 4개 feature 쿼리 전부 현재 활성 pod
  (`vllm-serving-84cd9b9fb7-7j2n9`) 기준 1.5초 미만 age.
- 기존 공식 완료 18건(load_ramp 15 + pod_kill 3) 전부의
  `results/trial-*.json` 실제 sha256이 state의 `result_hash`와
  글자 하나까지 일치(불일치 0건) - 직접 재계산으로 확인.
- 이번 턴 목표 6건 전부 `status=planned`(미착수) 확인.
- `--plan --from-run-id pod_kill-fixed_threshold-02-mainexp-v2
  --to-run-id pod_kill-fixed_threshold-03-mainexp-v2`로 정확히 6건·
  지정 순서만 미리보기됨을 확인. `--dry-run --resume`(읽기 전용,
  실제 클러스터 접근)으로 6건 전부 8개 preflight 항목 전부
  `ok=true`, `rollout_healthy_single_revision.classification==
  "healthy"` 기록 확인.

### 116.3 실행 (90분 제한 내, 실제 약 43분 소요)

시작 시각 기록 후 `--resume --from-run-id pod_kill-fixed_threshold-02-
mainexp-v2 --to-run-id pod_kill-fixed_threshold-03-mainexp-v2`(dry-run
아님)를 백그라운드 실행. 6건 전부 `outcome=recovered`/
`state=completed`로 중단 조건 없이 정상 종료
(`t_injection` 07:50:58Z ~ `t_recovery` 08:33:35Z, 총 약 43분 -
90분 한도 내, 새 trial 시작 없이 강제 종료도 없음).

| run_id | detector | detected | detection_source | decision_outcome | promotion_verified | audit_status |
|---|---|---|---|---|---|---|
| pod_kill-fixed_threshold-02-mainexp-v2 | fixed_threshold | True | reactive | executed_verified | True | complete |
| pod_kill-proposed-02-mainexp-v2 | isolation_forest | True | predictive | executed_verified | True | complete |
| pod_kill-native-02-mainexp-v2 | (없음) | False | - | - | - | - |
| pod_kill-proposed-03-mainexp-v2 | isolation_forest | True | predictive | executed_verified | True | complete |
| pod_kill-native-03-mainexp-v2 | (없음) | False | - | - | - | - |
| pod_kill-fixed_threshold-03-mainexp-v2 | fixed_threshold | True | reactive | executed_verified | True | complete |

`native-02`/`native-03`의 `detected`/`decision_outcome`/
`audit_record_id` 전부 `None`인 것은 §115.4에서 이미 확인한 native
arm의 정상 패턴과 동일(detector가 없는 arm이므로 감사할 결정 자체가
없음).

### 116.4 사후 검증 (매 trial 이후 실제로 확인)
...
- 6건 전부 `results/trial-pod_kill-*-mainexp-v2.json`의 실제
  `sha256sum`이 state의 `result_hash`와 정확히 일치(fixed_threshold-02:
  `cac3f064...`, proposed-02: `91c40c1c...`, native-02: `0692cdcf...`,
  proposed-03: `e37ceb95...`, native-03: `0523d5c7...`,
  fixed_threshold-03: `f496c5d7...`).
- `injection_valid=True`, `baseline_valid=True`, `probe_valid=True`
  6건 전부.
- detector 로그 4건(`fixed_threshold-02`/`proposed-02`/`proposed-03`/
  `fixed_threshold-03`) 전부 `Traceback`/`Error`/`Exception`/
  `DataGapFailClosed`/`prolonged_data_gap_invalidated`/
  `evaluation_skipped` 매치 0건 - 크래시·데이터 공백 없이 정상 종료.
- `fixed_threshold`·`proposed` 4건 전부 `audit_record_id` 발급,
  `audit_status=="complete"`.
- 최종 라이브 상태: `phase=Healthy`, 단일 revision(`755d5bdf7d`,
  6회 pod_kill로 pod 교체 - 정상), 1/1/1, pod 정확히 2개, Chaos CR
  잔여 0, 실험 pod 잔여 0, `/admin/quiescent`=true, `/admin/
  experiment-run`=null.

### 116.5 보존/범위 확인

- §115의 1차 반복 3건(`native-01`/`fixed_threshold-01`/`proposed-01`)
  결과·hash 변경 0건(재확인: 3건 hash 전부 §115 기록값과 동일).
...
- `pod_kill` 나머지 반복 4~5(`native-04/05`,
  `fixed_threshold-04/05`, `proposed-04/05`)는 `status=planned` 그대로,
  `network_degrade`·memory auxiliary는 시작하지 않음.
- 이번 턴 동안 원격에 `recovery-policy-bot`이 만든 audit-log 커밋
  23건(§113.11/§115와 동일한 종류, `audit-log/*.jsonl`만 변경)이
  들어와 있어 `git pull`(fast-forward 아닌 merge, 충돌 없음)로 통합 후
  이 문서 커밋과 함께 푸시.
...
## §124 - `network_degrade mainexp-v1` 2차 반복 실행 성공(3/3)

### 124.1 사전점검 - 전부 통과 (2026-09-24)

`HEAD==origin/master==924e83d`(§123 커밋), 추적 파일 변경 0건. §120/
§121/§123 수정 커밋(`3f622a1`/`11d3290`/`a7ae6a5`) 전부 `git merge-base
--is-ancestor`로 현재 HEAD의 조상임을 확인 - 그 위에 실제 소스코드도
직접 grep으로 재확인(`NETWORK_TOLERANT_PROBE_PROFILE`/`NETWORK_
TOLERANT_READINESS_TIMEOUT_SEC` 상수, `real_run_trial()`의 `--probe-
profile`/`--readiness-probe-timeout-sec` 추가 로직, 재검증 루프의
`stored_evidence` 재사용 로직 - 전부 현재 파일에 실존). `official-
experiment-state.json`(v1) 완료 24건 + `-v2.json`(v2) 완료 30건 = 54건
전부 hash 일치. `replacements`의 `network_degrade-native-01-mainexp-v1`
링크(→`retry1`, evidence 보존) 그대로. 두 Node Ready, Rollout 단일
revision(`768fdcc4d5`, base, 1/1/1), 재시작 0, Chaos CR 0, 실험 pod 0,
detector 프로세스 0, context null, quiescent true. 포트포워드 재기동 후
recovery-policy·Prometheus 정상, 4개 feature 쿼리 전부 1.5초 미만
age, 모델 v3.2b 해시 일치.

### 124.2 `--plan`/`--dry-run` - 정확한 3건 범위 확인

이번 턴 범위는 `--from-run-id network_degrade-fixed_threshold-02-
mainexp-v1`(직전 완료 trial인 `proposed-01`이 아니라 이번에 새로 실행할
**첫** trial을 지정 - `--from-run-id`는 그 지점을 포함해서 슬라이싱하므로
완료된 trial을 다시 넣으면 안 됨)로 지정. `--plan`: 정확히 `fixed_
threshold-02`(34)/`proposed-02`(35)/`native-02`(36) 3건만 미리보기됨을
확인. `--dry-run`(읽기전용, 실클러스터 접근): 3건 전부 8개 preflight
항목 `ok=true`.

### 124.3 base→tolerant 전환 - 직접 재확인 완료

시작 시각 기록(13:28:02Z) 후 백그라운드 실행. §100/§119/§123과 동일하게
독립적으로 직접 재확인: 새 preview pod(`vllm-serving-5f88b9f697-*`,
§100.4의 결정론적 해시 재현 - §123에서 tolerant로 썼던 것과 동일한
해시) Ready(promote까지 포함, `phase=Healthy`) -> 실측
`readinessProbe.timeoutSeconds=11`/`livenessProbe.timeoutSeconds=11`
확인 -> 구 revision(`768fdcc4d5`) 0/0/0 scale-down 확인 - trial 1의
injection 시작 전 완전히 끝남. (전환 직후 새 RS `fb98c5f46`가 곧바로
나타난 것은 `fixed_threshold-02` 자신이 promotion에 대비해 만드는
standby preview 사전 준비 - §123.6의 `proposed-01`에서도 같은 패턴을
확인한 정상 동작.)

### 124.4 실행 결과 - 3/3 전부 정상 완료

| run_id | detector_process | detector(실제 판단) | detection_source | outcome | postflight | hash 일치 |
|---|---|---|---|---|---|---|
| network_degrade-fixed_threshold-02-mainexp-v1 | fixed_threshold | alertmanager | reactive | recovered | 8/8 ok | O |
| network_degrade-proposed-02-mainexp-v1 | isolation_forest | alertmanager | reactive | recovered | 8/8 ok | O |
| network_degrade-native-02-mainexp-v1 | (없음) | (없음) | - | recovered | 8/8 ok | O |

세 trial 모두 `readiness_probe_profile="network_tolerant"`,
`readiness_probe_timeout_sec=11.0`가 결과 파일에 실측 기록됨.

**`fixed_threshold-02`는 `detector_process`/`detector`/`detection_
source`가 전부 일관됨**(`fixed_threshold`/`alertmanager`/`reactive`) -
애초에 reactive 전용 arm이므로 구별할 격차 자체가 없다.

**`proposed-02`는 `detector_process=isolation_forest`인데 실제
`detector=alertmanager`/`detection_source=reactive`** - `proposed-01`
(§123.6)과 동일한 패턴이 2/2로 재현됐다. 지시대로 **이 둘을 구별해
그대로 기록**하고, 왜 isolation_forest가 이번에도 예측적으로 탐지하지
못했는지는 **단정하지 않는다** - §123.6에서 제시했던 "network_degrade
의 feature 공간에 지연 신호가 없을 수 있다"는 가설은 여전히 하나의
가능한 설명일 뿐, 이 2건만으로 확정할 근거는 아니다(반복 3~5에서 같은
패턴이 계속되는지, `collect_metrics`의 `detector_check="reactive_
fallback"` 분류가 매번 유지되는지 추가 관찰이 필요하다는 뜻으로 남겨
둔다). `collect_metrics.build_comparison()`으로 재확인한 결과 이번에도
`detector_check="reactive_fallback"`, `included_in_main_analysis=True`,
`timing_anomaly=False`, 검증 issue 0건 - 데이터 결함 아님.

### 124.5 사후 검증 (매 trial마다 실제로 확인)

- 3건 전부 postflight 8개 항목 전부 `ok=true`,
  `rollout_healthy_single_revision.classification=="healthy"`,
  `cleanup_status=="ok"`.
- 3건 전부 실제 `sha256sum`이 state의 `result_hash`와 일치.
- `injection_valid=True`, `target_replaced=False` 3건 전부.
- detector 로그 2건(`fixed_threshold-02`/`proposed-02`) 전부
  `Traceback`/`Error`/`Exception`/`DataGapFailClosed`/`prolonged_data_
  gap_invalidated`/`evaluation_skipped` 매치 0건.
- `detection_stage`/`action_stage` 필드 모두 채워짐(`stage-4-4000ms`).

### 124.6 종료 후 복원 - 직접 재확인 완료

- `kubectl get rollout`: `phase=Healthy`, 단일 revision(`activeSel==
  previewSel`), 잔여 preview 없음.
- 활성 pod 실측 `readinessProbe.timeoutSeconds=1`/`livenessProbe.
  timeoutSeconds=1`(base 정상 복원), `restartCount=0`.
- `kubectl diff -f gitops/apps/vllm-serving/rollout.yaml` = 0(exit 0).
- Chaos CR 0건, `/admin/experiment-run`=null.
- `EXIT_CODE_MARKER=0`(실제 로그 마커로 재확인, `SequenceAborted` 없이
  정상 종료).
...
- 유효 완료 trial: **3/3**(`fixed_threshold-02`/`proposed-02`/
  `native-02` 전부 `recovered`/`completed`).
- 연결 안정성: 전 과정(약 48분)에서 kubectl/API 연결 끊김 없음.
- 총 소요 시간: 약 48분(13:28:02Z ~ 14:16:19Z) - 90분 한도 이내.
- 최종 클러스터 상태: `Healthy`, 단일 revision(base, timeout=1/1),
  재시작 0, Chaos CR 0, 실험 pod 0, context null, Git/live diff 0.
- 원본 `network_degrade-native-01-mainexp-v1`(`failed`) 및 기존 54건
  (v1 24건+v2 30건) 공식 결과·hash 변경 0건.
- `network_degrade` 상태: 6건 완료(1~2차 반복) + 1건 `failed`(원본,
  대체 연결·보존) + 9건 `planned`(반복 3~5).
...
## §139 - §138 정정 + 계측·분석 완결성 확보 (읽기전용, 라이브 실행 0건)

전체 보고서는 `docs/design/phase8-followup-correction-138.md`. 이번엔
코드 수정·오프라인 테스트·기존 evidence 재분석만 했다 - 클러스터 접속·
부하 생성·추가 trial·재시도 전부 0건.

**§138 정정 핵심**: 사용자가 t_detection이 t_recovery보다 fixed
329.765초/proposed 176.082초 뒤라고 지적 - 실측으로 정확히 일치함을
확인. 더 나아가 `analyze_followup_results.py`가 evidence 전체(주입 전
baseline 포함, 904/905건)를 "900초 cohort"라고 잘못 부른 것을 발견해
`t_injection<=sent_at<t_injection+900` 경계를 명시 적용한 `correct_
followup_138.py`(신규)로 재구성 - 독립 재계산값(fixed 900/900/1/69,
proposed 900/900/0/60)과 정확히 일치. §138이 보고한 "미완료 1건"은
실제로는 900초 cohort **밖**(사후 발신)이었음을 확인 - cohort 안은
두 arm 다 0건.

**신규 발견**: 재위반 탐색 로직을 처음 구현했을 때 원본 t_slo/t_recovery와
수 분 단위로 어긋나는 버그가 있었다 - 원인은 주입 전 이력(느린 표본
1.5s/0.9s)을 rolling window 계산에서 통째로 제외한 것(원본 판정은
not_before를 판정 시작 자격에만 적용하고 window 내용은 안 자름). 고친
뒤 재확인한 결과 **두 arm 모두 900초 안에 실제 재위반(episode 2)이
있었고, 원래 §138이 "회복과 무관한 사후 탐지"라고 결론지었던 t_detection/
t_switch가 사실은 이 재위반 구간 안에 있었다** - 인과는 여전히 미확정
이지만 "무관"이라는 §138의 결론은 불완전했다.

**계측 수정**: probe_followup.py에 기존 detector와 같은 --stop-file
패턴을 추가하고, load_ramp_followup_adapter.py의 stop()을 "발신중단→
drain 대기(최대 90초)→최종본 회수→정리" 순서로 재작성 - 이전엔 evidence만
회수하고 바로 강제종료해 진행 중 요청이 grace 없이 끊길 경합이 있었다.
비용 샘플러에도 "정상종료 후 꼬리"와 "진짜 mid-run 수집실패"를 구분하는
reclassify_missed() 추가 - 재분류 결과 기존에 "누락"으로 잡혔던 13/15건
전부 진짜 실패가 아니라 정상종료 꼬리였다.

**오프라인 검증**: 신규 10+1개 테스트(합성 fixture + 실제 로컬 서버) -
재위반 fixture는 최초 시도에서 window 간격을 너무 좁게 잡아 실패했다가
(60초 rolling window가 위반 표본을 씻어내는 데 걸리는 시간을 과소평가)
재설계해 통과. 전체 회귀 773/773(3 skip), 기존 45건 분석 출력은 byte
단위로 동일 - 새 파라미터가 기존 호출부에 전혀 영향 없음 확인.

**범위 확인**: 원본 결과·state·CSV·evidence·cost·contract 파일 전부
미변경(읽기만 함), 라이브 실행 0건, 지표별 사용가능/제한/불가 판정을
자동 일괄 처리하지 않고 개별 판정. 다음 반복 시간 계획도 "세션당
2쌍(4trial)"이 아니라 "세션당 1쌍"으로 하향 재추정(§138의 낙관적
추정 정정) - 이번 승인 범위에 다음 반복은 포함하지 않았다.

## §140 - 후속 비교 분석 계약 동결 + 첫 쌍 포함 기준 사전 고정

전체 문서는 `docs/design/phase8-followup-contract-140.md`. 다음 쌍
(proposed→fixed_threshold, rep02) 실행 **전에** 커밋 - 결과를 본 뒤
지표·창·포함기준을 바꾸지 않기 위함.

**표현 규칙 고정**: "전환이 첫 회복을 만들었다"/"전환이 모든 저하와
무관했다" 둘 다 금지 - "두 번째 episode 구간 안에 탐지·전환이 있다는
시간적 대응 관계가 있으나 인과 효과는 확정하지 않는다"로만 기술.

**분석 계약**: 주 비교=900초 cohort의 실패율/지연초과율/합집합율(둘 다
분모 표기), 보조 비교=전체구간 raw 위반시간·episode별 시간선(전부,
첫 episode만 대표시키지 않음)·재위반/미회복 여부·관측구간 detector
비용·실제 탐지경로/전환횟수. episode 재구성은 주입 전 이력을 rolling
window엔 유지하되 집계 구간만 주입 이후로 제한(§139에서 확립한 방식
그대로, 새 로직 없음). 비용은 배수+절대증가량 항상 병기, RSS 평균을
"누적 비용"이라 부르지 않음.

**첫 쌍 포함 기준 사전 고정**: 4개 질문에 전부 사전 답변 - cohort
완전 복원(예, §139 확인), 비용 계산법 비교가능(관측구간만 예), stop-file
수정이 in-window 동작을 바꾸는지(코드로 확정 - 아니오, stop()은
t_injection+900s 이후에만 호출됨), 종료 이후 보완인지(예). 결론: 900초
cohort 기반 지표는 포함 후보로 확정, 준비/정리 비용은 계속 별도 기술
통계로만. 기술적 통과조건 2개(drain 성공, cohort 이상 0건)를 다음 쌍
실행 후 기계적으로 확인하기로 사전 고정.

**범위 확인**: 클러스터 접속 0건(이 문서는 계약만), 결과를 안 본 상태에서
작성.

## §141 - 두 번째 쌍(proposed→fixed_threshold, rep02) 실행 결과

전체 문서는 `docs/design/phase8-followup-pair2-141.md`. §140 계약대로
proposed 1회→fixed_threshold 1회(첫 쌍과 반대 순서) 실행, 새 run_id
(`-02-`)로 기존 파일 무변경 보존. 실행 전 stop-file 메커니즘만 실제
클러스터에서 짧게 스모크 테스트(장시간 장애 pilot 아님) - touch 후
약 4.5초 만에 자연 종료 확인.

**§140 기술적 통과조건 2개 - 둘 다 PASS**: 두 trial 모두 drain 정상종료
(강제종료 없음), cohort 원인불명 이상 0건 - stop-file 수정이 실제
클러스터에서도 의도대로 동작함을 확인.

**핵심 발견 - 이점이 일관되지 않음**: rep01에서는 proposed가 실패율·
지연초과율 둘 다 낮았지만, rep02에서는 실패율이 동률(둘 다 0.11%)이
됐고 지연초과율은 오히려 fixed_threshold가 근소 우위(5.67% vs 6.00%)
였다 - n=2 시점에 이미 우위 방향이 바뀜, 정직하게 보고.

**추가 발견**: proposed rep02는 900초 안에 episode가 3개(rep01은 2개)
발생했고, 탐지(회복+0.73초)·전환(회복+11.67초)이 rep01(재위반 구간
안)과 달리 이번엔 재위반 **이전** 빈 구간에 있었다 - 같은 arm인데도
탐지·전환과 episode의 시간적 관계 자체가 반복마다 다름을 확인.
audit-log 전수 확인 결과 detector가 episode 2·3 구간에서 6번 더
이상 신호를 냈지만 idempotency_key(`{run_id}:{signal_type}`) 설계상
trial당 첫 신호만 조치로 이어질 수 있어 전부 `skipped_duplicate`
처리됨 - proposed 쪽의 실제 구조적 한계로 기록.

비용: CPU 배율 2.91배(rep01은 5.79배), 메모리 배율 3.92배(rep01은
4.23배) - "더 크다"는 방향은 일관되지만 배율 자체는 반복마다 다름.
